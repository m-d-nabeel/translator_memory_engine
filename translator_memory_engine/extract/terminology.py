...
def extract_terminology(chapters: List[Chapter], min_support: int = 2) -> List[Signal]:
    ...
    for ch in chapters:
        ...
        for m in _BRACKET_PATTERN.finditer(text):
            ...
            signals.append(Signal(
                text=term, chapter=ch.chapter, type="terminology",
                context=ctx, extractor="terminology.bracket",
            ))
        ...
        for m in _SINGLE_QUOTE_TERM.finditer(text):
            ...
            signals.append(Signal(
                text=term, chapter=ch.chapter, type="terminology",
                context=ctx, extractor="terminology.quoted",
            ))
            term_chapters.setdefault(term, set()).add(ch.chapter)
        ...
        for m in _POSSESSIVE_PATTERN.finditer(text):
            ...
            signals.append(Signal(
                text=term, chapter=ch.chapter, type="terminology",
                context=ctx, extractor="terminology.possessive",
            ))
            term_chapters.setdefault(term, set()).add(ch.chapter)
    ...
    if min_support > 1:
        passing = {
            t for t, chaps in term_chapters.items()
            if len(chaps) >= min_support
        }
        signals = [
            s for s in signals
            if s.text in passing
        ]
