...
_STOP_WORDS: Set[str] = {
    ...
    "Who", "Whom", "Why", "How", "However", "Although", "Also", "Already",
    "After", "Again", "And", "Another", "Any", "Are", "Around",
    "Before", "Being", "Between", "Both", "But", "Can", "Could",
    "Did", "Does", "Don", "During", "Each", "Either", "Else", "Even",
    "Every", "For", "From", "Get", "Got", "Had", "Has", "Have", "Her",
    "Here", "Him", "His", "If", "Into", "Its", "Just", "Let", "Like",
    "May", "More", "Most", "Much", "Must", "Never", "Not", "Now",
    "Once", "Only", "Other", "Our", "Out", "Over", "Own", "Perhaps",
    "Please", "Rather", "Really", "Same", "She", "Should", "Since",
    "So", "Some", "Still", "Such", "Sure", "Than", "Thank", "Too",
    "Upon", "Very", "Was", "Well", "Were", "Will", "With", "Would",
    "Yet", "You", "Your",
    # Common dialogue/narration starters
    "Yes", "No", "Oh", "Ah", "Hmm", "Huh", "Hey", "Hehe", "Haha",
    "Wow", "Ugh", "Tsk", "Ahem", "Phew", "Eek", "Gulp",
    # Common non-entity capitalized words in fiction
    "Chapter", "Part", "Volume", "Book", "Act", "Scene",
    "Sir", "Lord", "Lady", "Count", "Countess", "Duke", "Duchess",
    "Prince", "Princess", "King", "Queen", "Emperor", "Empress",
    "Viscount", "Baron", "Marquis",
    # Domain-suffix words — valid as suffixes, NOT as standalone entities
    "Sect", "Palace", "Hall", "Peak", "Clan", "Court", "Kingdom",
    "Empire", "Tower", "Mountain", "Valley", "Gate", "Formation",
    "Pill", "Art", "Technique", "Realm", "Stage", "Academy", "Guild",
    "Order", "Temple", "Church", "Castle", "Estate", "Manor",
    "Company", "Group", "Village", "City", "Province", "Territory",
    "Forest", "Lake", "River", "Island", "Fortress", "Fort",
    "Regiment", "Corps", "Legion", "Brigade", "Squad",
    "Sword", "Blade", "Shield", "Spear", "Staff",
    # Common fiction nouns that appear capitalised but aren't proper names
    "Hand", "Hands", "Care", "Disease", "Farm", "Field", "Money",
    "Deal", "Diet", "Earth", "God", "Goddess", "Gods", "High",
    "Sea", "Opening", "Resort", "Restaurant", "Trade", "Trading",
    "Merchant", "Turns", "Appeared", "Appears", "Acquiring",
    "Incurable", "Postpartum", "Extermination", "Uninvited",
    "Guests", "Mercenary", "Lords", "Watching", "Observing",
    "Seeing", "Behind", "Under", "Unlike", "Regarding",
    "Master", "Monster", "Monsters",
    "Knight", "Knights", "Soldier", "Soldiers", "Warrior", "Warriors",
}
...
_STOP_PREFIXES: Set[str] = {
    "As", "At", "But", "If", "In", "Is", "Or", "So", "To",
    "By", "Of", "On", "An", "Do", "Up",
    "Even", "Although", "After", "Before", "Behind", "During",
    "Despite", "Since", "While", "When", "Where", "Whether",
    "Under", "Unlike", "Until", "Upon", "Within", "Without",
    "Watching", "Observing", "Seeing", "Regarding", "Following",
}
...
_TITLE_PREFIXES: List[str] = [
    "Lord", "Lady", "Sir", "Count", "Countess", "Duke", "Duchess",
    "Prince", "Princess", "King", "Queen", "Emperor", "Empress",
    "Viscount", "Baron", "Baroness", "Marquis", "Master", "Mistress",
    "Captain", "Commander", "General", "Chief", "Elder",
    "Saint", "Prophet", "Sage", "Grand",
]

# Pre-compiled patterns
_TITLE_PATTERN = re.compile(
    r'\b(' + '|'.join(re.escape(t) for t in _TITLE_PREFIXES) +
    r')\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b'
)
_MULTI_CAP_PATTERN = re.compile(
    r'\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)\b'
)
_SINGLE_CAP_PATTERN = re.compile(
    r'\b([A-Z][a-z]{2,})\b'
)
    ...
    for ch in chapters:
        ...
        for m in _TITLE_PATTERN.finditer(text):
            ...
            signals.append(Signal(
                text=full, chapter=ch.chapter, type="entity",
                context=ctx, extractor="entity.title_name",
            ))
            # Also emit the bare name as a signal
            if name not in _STOP_WORDS:
                signals.append(Signal(
                    text=name, chapter=ch.chapter, type="entity",
                    context=ctx, extractor="entity.title_name_bare",
                ))
        ...
        for m in _SUFFIX_PATTERN.finditer(text):
            ...
            ctx = _get_sentence_context(text, m.start(), m.end())
            signals.append(Signal(
                text=phrase, chapter=ch.chapter, type="entity",
                context=ctx, extractor="entity.domain_suffix",
            ))
        ...
        for m in _MULTI_CAP_PATTERN.finditer(text):
            ...
            signals.append(Signal(
                text=phrase, chapter=ch.chapter, type="entity",
                context=ctx, extractor="entity.multi_cap",
            ))
        ...
        for m in _SINGLE_CAP_PATTERN.finditer(text):
            ...
            ctx = _get_sentence_context(text, m.start(), m.end())
            signals.append(Signal(
                text=word, chapter=ch.chapter, type="entity",
                context=ctx, extractor="entity.single_cap",
            ))

    # Filter single-word names by min_support (must appear in multiple chapters)
    if min_support > 1:
        passing_singles = {
            name for name, chaps in single_word_chapters.items()
            if len(chaps) >= min_support
        }
        signals = [
            s for s in signals
            if s.extractor != "entity.single_cap" or s.text in passing_singles
        ]
