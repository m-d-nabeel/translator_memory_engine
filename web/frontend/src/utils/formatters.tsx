export function isIdentityPolicy(
  actionStr: string,
  triggerStr?: string,
): boolean {
  ...
  try {
    ...
    if (
      data.render_as &&
      data.render_as.trim().toLowerCase() === triggerStr.trim().toLowerCase()
    ) {
      return true;
    }
  } catch {
...
export function formatPolicyAction(actionStr: string, triggerStr?: string) {
  if (!actionStr)
    return (
      <span className="opacity-60 italic text-xs">No action specified</span>
    );
  try {
    const data = JSON.parse(actionStr);
    if (data.render_as) {
      if (
        ...
        return (
          ...
          >
            Protected exact canonical name:{" "}
            <strong className="font-mono text-[var(--color-accent)]">
              {data.render_as}
            </strong>
          </span>
        );
    ...
    if (data.pattern || data.replacement) {
      return (
        <div className="flex flex-wrap items-center gap-1.5 mt-1 text-xs">
          <span className="opacity-80" style={{ color: "var(--color-text)" }}>
            Replace
          </span>
          <code
    ...
    return (
      <pre
        className="font-mono text-[11px] overflow-x-auto mt-1"
        style={{ color: "var(--color-text)" }}
      >
...
export function formatAliasesList(
  ...
  try {
    ...
    const filtered = canonical
      ? arr.filter(
          (a: string) =>
            a.trim().toLowerCase() !== canonical.trim().toLowerCase(),
        )
      : arr;
    ...
    return (
      ...
      >
        <span
          className="text-[10px] uppercase font-bold opacity-65 block mb-1.5"
    ...
    return (
      ...
      >
        <span
          className="text-[10px] uppercase font-bold opacity-65 block mb-1"
          style={{ color: "var(--color-text-muted)" }}
        >
          Variants & Aliases
        </span>
