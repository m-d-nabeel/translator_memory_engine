...
interface PasteFormProps {
  onSubmit: (data: { chapter_number: number; source_type: string; raw_text: string }) => Promise<void>;
  isProcessing: boolean;
  nextSuggestedNumber?: number;
  onCancel?: () => void;
}

export function PasteForm({ onSubmit, isProcessing, nextSuggestedNumber = 1, onCancel }: PasteFormProps) {
  const [chapterNumber, setChapterNumber] = useState(String(nextSuggestedNumber));
    ...
    >
      <div className="flex items-center justify-between border-b pb-4" style={{ borderColor: "var(--color-border)" }}>
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center text-white" style={{ background: "linear-gradient(135deg, var(--color-accent) 0%, #ea580c 100%)" }}>
            <Sparkles className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-sm md:text-base font-bold font-outfit" style={{ color: "var(--color-text)" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <div className="relative flex items-center">
            <span className="absolute left-3 text-xs font-bold font-mono opacity-50" style={{ color: "var(--color-text-muted)" }}>Ch.</span>
            <input
              type="number"
              value={chapterNumber}
        ...
        <div>
          <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5 flex items-center gap-1.5" style={{ color: "var(--color-text-muted)" }}>
            <Languages className="w-3.5 h-3.5 text-[var(--color-accent)]" />
            Source Stream
          </label>
          <div className="grid grid-cols-2 gap-1.5 p-1 rounded-xl border bg-black/20" style={{ borderColor: "var(--color-border)" }}>
            <button
              type="button"
              onClick={() => setSourceType("mtl")}
              className={`py-2 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                sourceType === "mtl" ? "shadow-sm" : "opacity-60 hover:opacity-100"
              }`}
              style={{
                backgroundColor: sourceType === "mtl" ? "var(--color-surface-hover)" : "transparent",
                color: sourceType === "mtl" ? "var(--color-ai)" : "var(--color-text-muted)",
              }}
            >
              Raw MTL
            </button>
            <button
              ...
              className={`py-2 px-3 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                sourceType === "original" ? "shadow-sm" : "opacity-60 hover:opacity-100"
              }`}
              style={{
                backgroundColor: sourceType === "original" ? "var(--color-surface-hover)" : "transparent",
                color: sourceType === "original" ? "var(--color-text)" : "var(--color-text-muted)",
              }}
            >
              Original Text
            </button>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className="text-[11px] font-mono opacity-70" style={{ color: "var(--color-text-muted)" }}>
        ...
        <textarea
          ...
          placeholder={
            sourceType === "mtl"
              ...
              : "Paste original source text (Korean, Chinese, Japanese) here..."
          }
          required
        />
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <p className="text-[11px] opacity-70 hidden sm:block max-w-sm" style={{ color: "var(--color-text-muted)" }}>
          💡 <strong style={{ color: "var(--color-text)" }}>Pro Tip:</strong> Upon processing, new translation policies will be automatically extracted and indexed.
        </p>
        <button
          type="submit"
